...
import { getAsk, listAsks } from "../graphql/queries.js";
import { createAsk, deleteAsk } from "../graphql/mutations.js";
...
// add ask price for certain token
router.put<
  null,
  any,
  {
  ...
  async (req, res) => {
    try {
      ...
      const result = await client.graphql({
        query: listAsks,
        variables: {
          filter: { and: [{ price: { le: price } }, { mint: { eq: mint } }] },
          limit: 1,
          sortDirection: "DESC",
        },
      });
      ...
      const isMatch =
        (result as { data?: any })?.data?.listAsks?.items?.length > 0;
      ...
      if (isMatch) {
        ...
        res.statusCode = 200;
        res.send({
          id: (result as { data?: any })?.data?.listAsks?.items[0].id,
        });
      } else {
        res.statusCode = 200;
        res.send(null);
...
router.get<{ id: string }>("/:id", async (req, res) => {
  try {
    ...
    // return result
    res.statusCode = 200;
    res.send((result as { data: any }).data.getAsk);
  } catch (err) {
    // warn the error in the console
    console.warn(err);
...
router.get<{ mint: PublicKey }>("/mint/:mint", async (req, res) => {
  try {
    ...
    // return result
    res.statusCode = 200;
    res.send((result as { data: any }).data.listAsks.items);
  } catch (err) {
    // warn the error in the console
    console.warn(err);
